...
/* this is for stage2 - using a linked list we make things more effecient */
...
void print_data(char data[NOMAXELEMENTS][MAXCHAR], FILE *write_file)
{
    fprintf(write_file, 
        ...
        data[TEAM],data[NOC],data[GAMES],data[YEAR],data[SEASON],data[CITY],
        data[SPORT], data[EVENT], data[MEDAL]);
}
...
node_t *bstSearch(node_t *parent, char input[MAXCHAR], FILE *write_file)
{
    ...
    /* if not found the print NOTFOUND as required */
    if (not_found)
    {
